...
from .utils import handle_excel_file, marketing_email
...
def compose_newsletter(request):
    if request.method == 'POST':
        ...
        if news_form.is_valid():
            marketing_email(subject=news_form.subject, message=news_form.message)
            return HttpResponseRedirect('/')
    else:
        news_form = NewsletterForm()

    return render_to_response('coffeeclubapp/compose_newsletter.html', {'email_form':news_form},
                    context_instance=RequestContext(request))
